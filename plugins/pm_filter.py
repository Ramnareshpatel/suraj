...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("ʙᴀᴄᴋ", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"🌺 ᑭᗩᏀᗴ {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
    elif off_set is None:
        btn.append([InlineKeyboardButton(f"ᴘᴀɢᴇ {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages"), InlineKeyboardButton("ᑎᗴ᙭Ꭲ ⏩", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"ᴘᴀɢᴇ 1/{round(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="ᑎᗴ᙭Ꭲ ⏩",callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
