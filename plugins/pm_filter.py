...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏪ ᗷᗩᑕᏦ", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"🌺 ᑭᗩᏀᗴ {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
    elif off_set is None:
        btn.append([InlineKeyboardButton(f"🌺 ᑭᗩᏀᗴ {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages"), InlineKeyboardButton("ᑎᗴ᙭Ꭲ ⏩", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
                InlineKeyboardButton("⏪ ᗷᗩᑕᏦ", callback_data=f"next_{req}_{key}_{off_set}"),
                InlineKeyboardButton(f"🌺 ᑭᗩᏀᗴ {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages"),
                InlineKeyboardButton("ᑎᗴ᙭Ꭲ ⏩", callback_data=f"next_{req}_{key}_{n_offset}")
            ],
        )
    try:
    ...
    if query.data == "close_data":
        ...
        if chat_type == "private":
            ...
            if (st.status == "creator") or (str(userid) in ADMINS):
                ...
                await query.answer("ᴛʜᴀᴛs ɴᴏᴛ ꜰᴏʀ ʏᴏᴜ!!",show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('ᴄʜᴇᴄᴋ ᴘᴍ, ɪ ʜᴀᴠᴇ sᴇɴᴛ ꜰɪʟᴇs ɪɴ ᴘᴍ',show_alert = True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("🥺ꜰʀɪᴇɴᴅ...ᴛʜᴀᴛ's ɴᴏᴛ ꜰᴀɪʀ 😓 ᴘʟᴇᴀsᴇ ᴊᴏɪɴ ᴛʜᴇ ᴄʜᴀɴɴᴇʟ..🥺",show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        buttons = [[
            ...
            InlineKeyboardButton('☢️ ᴀᴜᴛᴏ ꜰɪʟᴛᴇʀ', callback_data='autofilter')
            ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"🌺 ᑭᗩᏀᗴ 1/{round(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="ᑎᗴ᙭Ꭲ ⏩",callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
            [InlineKeyboardButton(text="🌺 ᑭᗩᏀᗴ 1/1",callback_data="pages")]
        )
